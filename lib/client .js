...
try {
    ...
    console.log("🌍 Connected to the SENU DB");
} catch {
...
let cc = Config.sessionName.replace(/QUEEN-SENU-MD;;;/g, "").replace(/QUEEN-SENU-MD;;;/gi, ""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
...
await updateStatus()
 ...
 Void.ev.on('group-participants.update', async(anu) => {
                    ...
                    if (checkinfo) {
                        ...
                        if(/@pp/g.test(goodbye_messages)) {
                           let buttonMessage = {
                            ...
                            let buttonMessage = {
                                image: { url : ppUrl },
                                caption: `[ 𝗗𝗘𝗠𝗢𝗧𝗘 𝗛𝗔𝗦 𝗕𝗘𝗘𝗡 𝗗𝗘𝗧𝗘𝗖𝗧𝗘𝗗 ]\n\nName : @${num.split("@")[0]}\nStatus :from Admin ◈ Member`,
                                footer: `Izuku`,
                                mentions: [num],
                                headerType: 4,
                            }
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    console.log("✅ QUEEN SENU MD Login Successful!");
                    console.log("⬇️  QUEEN SENU MD Installing External Plugins...");
                    ...
                    for (let i = 0; i < check.length; i++) {
                     ...
                     console.log("😁  QUEEN SENU External Plugins Installed!");fs.readdirSync(__dirname + "/../commands").forEach((plugin) => {
